...
Loads in __init__, generates in generate(). Init methods are inlined;
remaining functionality bound programmatically from handler modules.
"""
...
def _collect_dit_wrapper_bindings():
    bindings = {}
    for mod in [generation, conditioning, codec, utils]:
        for name in mod.__all__:
            bindings[name] = getattr(mod, name)
    return bindings


for _name, _obj in _collect_dit_wrapper_bindings().items():
    setattr(AceStepDiTWrapper, _name, _obj)
