...
class MetadataConstrainedLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        enabled: bool = True,
        debug: bool = False,
        genres_vocab_path: str | None = None,
        ...
        self.caption: str | None = None  # Set via update_caption() before each generation
        ...
        self.user_provided_metadata: dict[str, str | None] = {
            "bpm": None,
            "caption": None,
            "duration": None,
            "keyscale": None,
        ...
        self.metadata_temperature: float | None = None
        self.codes_temperature: float | None = None

        # Duration constraint for codes generation
        # 5 codes = 1 second, so target_codes = target_duration * 5
        self.target_duration: float | None = None  # User-specified duration in seconds
        self.target_codes: int | None = None  # Computed target codes count
        ...
        self.current_user_field: str | None = None  # Current field being injected
    ...
    def _get_next_field_state(self, current_field: str) -> FSMState | None:
        """
    ...
    def set_user_metadata(self, metadata: dict[str, str | None] | None = None):
        """
    ...
    def _precompute_tokens(self):
        ...
        self.audio_code_mask: torch.Tensor | None = None
        # Inverse mask: block all non-audio-code tokens (for CODES_GENERATION state)
        self.non_audio_code_mask: torch.Tensor | None = None
    ...
    def _extract_code_from_token(self, token_id: int) -> int | None:
        """
    ...
    def _load_genres_vocab(self):
        ...
        try:
            ...
            with open(self.genres_vocab_path, encoding='utf-8') as f:
                genres = []
                for line in f:
                    line = line.strip()
    ...
    def _extract_caption_genres(self, caption: str):
        ...
        if not matched_genres:
            if self.debug:
                logger.debug("No genres matched in caption, using full vocab")
            return
    ...
    def _get_genres_trie_node(self, prefix: str) -> dict | None:
        """
    ...
    def _get_trie_node_from_trie(self, trie: dict, prefix: str) -> dict | None:
    ...
    def _get_user_provided_field_tokens(self, field_name: str) -> list[int] | None:
        """
            ...
            if allowed:
                ...
                if self.state == FSMState.THINK_END_TAG and self.stop_at_reasoning:
                    ...
                    if self.eos_token_id is not None:
                        self._apply_whitelist_inplace(scores, [self.eos_token_id])
                        if self.debug:
                            logger.debug("stop_at_reasoning=True: forcing EOS after completing </think> tag")
                        return scores
        ...
        if self.state in self.next_state:
            ...
            if old_state == FSMState.THINK_END_TAG:
                if self.generation_phase == "understand":
                    ...
                    if self.debug:
                        logger.debug("generation_phase='understand': allowing free-form lyrics after </think>")
